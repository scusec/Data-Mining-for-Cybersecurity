...
def generate(odir,wdir,label):
    ...
    with open(odir, 'rb') as f:
        ...
        for line in data:
            ...
            if len(line)!=0:
                space_f=(line.count(" ")+line.count("%20")+line.count('+')+line.count('%2B'))/len(line)#空格百分比
                special_f=(line.count("{")*2+line.count("%7B")*2+line.count("}")*2+line.count("%7D")*2+line.count("(")*2+
                           line.count('%28')*2+line.count('%29')+line.count(")")*2+line.count('NULL')+line.count('[')+
                           line.count('%5B')+line.count(']')+line.count('%5D')+line.count('=')*10+line.count('%3D')*10+
                           line.count('?')+line.count("%3F") + line.count(',')*5* + line.count('%2C')*5 +
                           line.count("'")*3+line.count("%27")*3+line.count('"')*2 + line.count("%22")*2)/len(line)
