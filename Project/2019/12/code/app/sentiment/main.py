...
with codecs.open(datapath+r"pos.csv","r","utf-8") as f1,\
     codecs.open(datapath+r"neutral.csv","r","utf-8") as f2,\
     codecs.open(datapath+r"neg.csv","r","utf-8") as f3:
    for line in f1:
        positive_data.append(delstopword(" ".join(i for i in jieba.lcut(line.strip(),cut_all=False)), stopkey))
        y_positive.append([1,0,0])
    for line in f2:
        neutral_data.append(delstopword(" ".join(i for i in jieba.lcut(line.strip(),cut_all=False)), stopkey))
        y_neutral.append([0,1,0])
    for line in f3:
        negative_data.append(delstopword(" ".join(i for i in jieba.lcut(line.strip(),cut_all=False)), stopkey))
...
with codecs.open(datapath + r"vocabulary.txt","w","utf-8") as f:
...
with codecs.open(datapath+r"sgns.wiki.bigram","r","utf-8") as f:
    line=f.readline()
...
# 模型保存
model.save(datapath+r"sentiment_analysis_lstm.h5")
